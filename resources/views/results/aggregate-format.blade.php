<x-results :highload="$highload"
    ...
    <div class="text-lg">{{ __('Total number of responses') }}: {{ $total }}</div>
    ...
    <div class="relative flex pt-6 items-center">
        ...
        <span class="flex-shrink mx-4 text-slate-400 text-xs font-semibold uppercase tracking-widest"
            >{{ __('Responses grouped by HTTP status') }}</span>
        <div class="flex-grow border-t border-slate-200"></div>
    </div>
    ...
    <div class="p-6 mt-6 bg-white border border-slate-200 rounded-xl shadow-sm transition-shadow">
        <div>{{ __('HTTP status') }}: <span class="text-indigo-500">{{ $response->status }}</span></div>
        <div>{{ __('Number of responses in group') }}: <span class="text-green-500">{{ $response->responses_count }}</span></div>
    </div>
    @endforeach
</x-results>
